...
class Play4blue:
	...
	def update_screen(self,master,main_bg):
		...
		self.main_bg = tkinter.Label(master, bg= "#%02x%02x%02x" % (115, 190, 255))
	...
	def check_game_status(self):
		...
		if len(self.clicks) == 4:	
			...
			self.removeButtons()
			self.rgb = np.array([115.0, 190.0, 255.0])
	...
	def fadeColor(self):
		if self.reinforcement[-1]:
			self.rgb -= 0.01*(np.array([115.0, -10.0, 255.0]))
			...
			self.rgb -= 0.01*(np.array([115.0, 190.0, 255.0]))
