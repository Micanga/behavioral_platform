...
class Play4red:
	...
	def __init__(self, master, prev_sc, main_bg):
		...
		self.saved_order = deepcopy(self.order)
	...
	def update_screen(self,master,main_bg):
		...
		self.main_bg = tkinter.Label(master, bg= "#%02x%02x%02x" % (255, 158, 158))
	...
	def memory_game(self):
		...
		if self.clicks[-1] == self.memo_correct_answer:
			...
			self.memo_reinforced.append([self.memory[0],True])
		else:
			self.reinforcement.append(False)
			self.memo_accuracy = 0
			self.memo_reinforced.append([self.memory[0],False])

		# 2. Showing the reinforcement screen
		self.repeat += 1
		self.removeButtons()
		self.rgb = np.array([255.0, 158.0, 158.0])
	...
	def fadeColor(self):
		if self.reinforcement[-1]:
			self.rgb -= 0.01*(np.array([255.0, -42.0, 158.0]))
			...
			self.rgb -= 0.01*(np.array([255.0, 158.0, 158.0]))
	...
	def replay(self):
		...
		if self.repeat == 24:
			...
			if self.order[0] == 2:
				self.memory.pop(0)
				utils.write_result('4-Vermelho',self,False,True)
			else:
				utils.write_result('4-Vermelho',self,False,False)
			...
			if self.order[0] == 2:
				self.memory.pop(0)
				utils.write_result('4-Vermelho',self,True,True)
			else:
				utils.write_result('4-Vermelho',self,True,False)
				self.results.append(self.clicks)
				self.result_set.add(self.clicks)
				self.frequency[self.clicks] += 1 #frequency calculated later
				self.total_frequency[self.clicks] += 1
			...
			self.clicks = ''
			self.round_start_time = datetime.datetime.now()
			self.main_bg.configure(bg="#%02x%02x%02x" % (255, 158, 158))
			if self.order[0] == 2:
				if self.experiment != 1:
	...
	def createImgButtons(self):
		...
		if self.experiment == 1:
			...
			print(':: experiment number error ::Play4red::createImgButtons::360+:: exit 1 ::')
			exit(1)
	...
	def ableButtonsAndMouse(self):
		...
		if self.experiment == 1:
			...
			print(':: experiment number error ::Play4red::ableButtonsAndMouse::430+:: exit 1 ::')
			exit(1)
